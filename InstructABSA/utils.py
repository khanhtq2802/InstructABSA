...
class T5Generator:
    def __init__(
        ...
        self.device = 'cuda' if torch.backends.cuda.is_built() else ('mps' if torch.has_mps else 'cpu')
    ...
    def get_metrics(
            ...
            y_true : list, 
            y_pred : list, 
            is_triplet_extraction : bool = False):
        ...
        if not is_triplet_extraction:
            for gt, pred in zip(y_true, y_pred): 
                # gt is label of 1 instance, may be
                # 1. 'Firewire 800, USB3, USB3, MAC OS, Windows 7'
                # 2. 'noaspectterm'
                # 3. '"WLAN" card, LAN device drivers'
                # 4. third-generation CPU ("Ivy Bridge"), fourth-generation Haswell CPU
                # 5. built in screen size, external 24" LCD screen

                # pred maybe 
                # 1. 'feature, Firewire 800, USB3, Windows 7, USB3, USB3, USB3, USB3, USB3, USB3, Windows 7',
                # 2. 'noaspectterm'
                # 3. 'WLAN card, LAN device drivers'
                # 4. CPU, Haswell CPU
                # 5. screen, screen size
